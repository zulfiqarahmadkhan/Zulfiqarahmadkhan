...
# Remove dummy rank and impactfactor
$pubsContent = $pubsContent -replace "`r`n  rank=\{Q1\},`r`n  impactfactor=\{7\.5\},", ""
$pubsContent = $pubsContent -replace "`n  rank=\{Q1\},`n  impactfactor=\{7\.5\},", ""

# Now replace the JS logic to look for citation_count
$scriptStart = $pubsContent.IndexOf("      const citationMatch = publication.match(/citation={([^}]*)}/);")
...
$newContent = $pubsContent.Substring(0, $scriptStart) + $goodScript
